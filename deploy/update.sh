...
DEPLOYED_TAG_FILE="${DEPLOYED_TAG_FILE:-${DEPLOY_DIR}/.deployed-tag}"
...
if [[ "${SKIP_GIT_CHECKOUT}" != "1" ]]; then
  ...
  PREVIOUS_LABEL="$(cat "${DEPLOYED_TAG_FILE}" 2>/dev/null || git describe --tags --exact-match HEAD 2>/dev/null || git rev-parse --short HEAD 2>/dev/null || echo "uploaded-artifact")"
fi
